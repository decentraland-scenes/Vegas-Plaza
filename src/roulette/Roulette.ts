...
const roulette_table_shape = new GLTFShape(
  'models/roulette/roulette_table.glb'
);
const roulette_table_v2_shape = new GLTFShape(
  'models/roulette/roulette_table_v2.glb'
);
const roulette_wheel_shape = new GLTFShape(
  'models/roulette/roulette_wheel.glb'
);
...
ink
  ...
  constructor(
    ...
    var wheelDownEvent = new OnPointerDown(e => {
      if (!this.spinning) {
  ...
  select_coin() {
    this.text_chip_1.getComponent(
      TextShape
    ).value = this.chip_value1.toString();
    this.text_chip_2.getComponent(
      TextShape
    ).value = this.chip_value2.toString();
    this.text_chip_3.getComponent(
      TextShape
    ).value = this.chip_value3.toString();
    this.text_chip_4.getComponent(
      TextShape
    ).value = this.chip_value4.toString();
...
class RouletteUI {
  ...
  public showCountDownText(seconds) {
    ...
    if (seconds > -1) {
      ...
      this.countdownTimer.addComponentOrReplace(new utils.Interval(1000, () => {
...
export class Roulette {
  ...
  async messageHandler(json) {
    ...
    if (json.gameData.method == 'place_chip') {
      instances[myTableID].spawn_chips_on_number(json.gameData.bet, json.gameData.money_held, true);
      ...
      if (json.gameData.bet_array) {
        json.gameData.bet_array.forEach(element => {
          instances[myTableID].spawn_chips_on_number(element.bet, element.money_held, true);
        });
      }
    } else {
